    ...
    try {
      ...
      if (emailData.campaignId && emailData.contactId) {
        await this.logEmailEvent({
          ...
          contactId: emailData.contactId,
          eventType: 'bounced',
          eventData: { error: error instanceof Error ? error.message : String(error), email: emailData.to }
        })
      }
      ...
      for (let i = 0; i < bulkData.recipients.length; i += batchSize) {
        ...
        for (const recipient of batch) {
          try {
            ...
            results.failed++
            results.errors.push(`${recipient.email}: ${error instanceof Error ? error.message : String(error)}`)
          }
