// Enhanced Discovery Page with Selection and Save Functionality
'use client'
...
export default function EnhancedLeadDiscoveryPage() {
    ...
    const [savedCompanies, setSavedCompanies] = useState<DiscoveredLead[]>([])
    ...
    const [searchParams, setSearchParams] = useState({
        ...
        includeVCs: true,
        maxResults: 10 // Reduced since we're calling detail API for each
    })
    ...
    const loadSavedCompanies = async () => {
        try {
            const response = await fetchWithDemo('/api/saved-companies')
            if (response.ok) {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                toast.success(`Found ${data.leads?.length || 0} companies with enhanced details, ${data.totalContacts || 0} contacts, and ${data.vcContacts?.length || 0} VCs!`)
            } else {
    ...
    const handleSaveSelected = async () => {
        ...
        setIsSaving(true)
        try {
            const companiesToSave = filteredLeads.filter(lead => selectedCompanies.has(lead.id))

            const response = await fetchWithDemo('/api/saved-companies', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ companies: companiesToSave })
            })
            ...
            if (response.ok) {
                ...
                setSelectedCompanies(new Set())
                loadSavedCompanies() // Refresh saved companies
            } else {
                const errorData = await response.json()
                throw new Error(errorData.message || 'Failed to save companies')
            }
        } catch (error) {
            console.error('Save error:', error)
            toast.error(error instanceof Error ? error.message : 'Failed to save companies')
        } finally {
            setIsSaving(false)
        }
    }
        ...
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-gray-600">Find companies with complete details, founders, and VCs</p>
                </div>
                <div className="flex items-center space-x-4">
                    ...
                    >
                        ...
                        <span>Saved ({savedCompanies.length})</span>
                    </Button>
                    <Button
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Configure your discovery criteria - now with complete company details</CardDescription>
                </CardHeader>
                            ...
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <Input
                                        ...
                                        className="w-24 mt-1"
                                    />
                                    <p className="text-xs text-gray-500 mt-1">Lower for faster complete details</p>
                                </div>
                    ...
                    <CardContent className="p-6">
                        <div className="space-y-4">
                            ...
                            <p className="text-sm text-gray-600">
                                Getting complete company details including proper locations, funding info, and descriptions...
                            </p>
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <CardHeader>
                        <CardTitle className="flex items-center justify-between">
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                >
                                    {selectedCompanies.size === filteredLeads.length ? 'Deselect All' : 'Select All'}
                                </Button>
                                <Button
                                    onClick={handleSaveSelected}
                                    disabled={selectedCompanies.size === 0 || isSaving}
                                    className="flex items-center space-x-2"
                                >
                                    <Save className="w-4 h-4" />
                                    <span>{isSaving ? 'Saving...' : `Save Selected (${selectedCompanies.size})`}</span>
                                </Button>
                            </div>
                        </CardTitle>
                    </CardHeader>
            ...
            {(filteredLeads.length > 0 || vcContacts.length > 0) && (
                <Card>
                    <CardHeader>
                        <CardTitle>Enhanced Discovery Results</CardTitle>
                        <CardDescription>Companies with complete details and VCs with key decision makers</CardDescription>
                    </CardHeader>
                    <CardContent>
                                            ...
                                            <tr key={lead.id} className="border-b hover:bg-gray-50">
                                                ...
                                                <td className="p-3">
                                                    <div className="flex items-center text-sm text-gray-600">
                                                        ...
                                                        {/* ENHANCED LOCATION: City, Country from detailed API */}
                                                            ...
                                                            <Card>
                                                                <CardContent className="p-12 text-center">
                                                                    ...
                                                                    <p className="text-gray-600 mb-4">
                                                                        Find companies with complete details, proper locations, funding info, and key decision makers
                                                                    </p>
                                                                        ...
                                                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                                                            <div>
                                                                                ...
                                                                                <div className="space-y-2 text-sm">
                                                                                    ...
                                                                                    {/* ENHANCED: Full address from complete API */}
                                                        ...
                                                        <Dialog open={showSavedDialog} onOpenChange={setShowSavedDialog}>
                                                            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                                                                <DialogHeader>
                                                                    ...
                                                                    <DialogDescription>Your saved companies and contacts</DialogDescription>
                                                                </DialogHeader>
                                                                ...
                                                                <div className="space-y-4">
                                                                    {savedCompanies.length > 0 ? (
                                                                                ...
                                                                                <div key={company.id} className="p-4 border rounded-lg">
                                                                                    <div className="flex justify-between items-start">
                                                                                        <div className="flex items-start space-x-3">
                                                                                            ...
                                                                                            <div>
                                                                                                <h5 className="font-medium">{company.company}</h5>
                                                                                                <p className="text-sm text-gray-600">{company.location}</p>
                                                                                                <p className="text-xs text-gray-500">{company.contacts.length} contacts</p>
                                                                                            </div>
                                                                                        </div>
                                                                                        <div className="flex items-center space-x-2">
                                                                                            {company.ai_score && (
                                                                                                <Badge className={`${getScoreBadge(company.ai_score)} text-xs`}>
                                                                                                    {company.ai_score}
                                                                                                </Badge>
                                                                                            )}
                                                                                        </div>
                                                                        ...
                                                                        <div className="text-center py-8">
                                                                            ...
                                                                            <p className="text-gray-600">No saved companies yet</p>
                                                                            <p className="text-sm text-gray-500">Select companies from search results to save them</p>
                                                                        </div>
                                                                    )}
                                                                </div>
