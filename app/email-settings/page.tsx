...
export default function EmailSettingsPage() {
  ...
  const loadSettings = async () => {
    try {
      ...
      const response = await fetchWithDemo('/api/settings/email')
  ...
  const handleTestEmail = async () => {
    ...
    try {
      if (isDemoMode) {
        ...
        const response = await fetchWithDemo('/api/email/test-settings', {
          method: 'POST',
          headers: { 
      ...
      if (error instanceof Error) {
        if (error instanceof Error ? error.message : String(error).includes('JSON')) {
          errorMessage = 'API endpoint returned invalid response (likely HTML instead of JSON)'
        } else if (error instanceof Error ? error.message : String(error).includes('fetch')) {
